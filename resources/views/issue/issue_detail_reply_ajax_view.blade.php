...
@foreach($comments as $row)
    <div class="direct-chat-msg" >
        ...
        @if ($row->category == 'issue')
                ...
                <div class="direct-chat-text bg-white" style="border: none;font-weight: bold;font-size: 17px;">{{$row->details}} </div>
            ...
            @foreach($row->child as $child)
                <div style="padding: 5px; margin-bottom: 10px; border-top: solid 1px #ddd;">
                    <small class="text-bold">
                        <i class="fa fa-reply"></i><i class="fa fa-user-circle"></i> &nbsp;{{$child->name}} &nbsp;&nbsp;
                        <i class="fa fa-calendar"></i> &nbsp;{{$child->created_at}} &nbsp;&nbsp;
                        <i class="fa fa-tags" title="Refrence ID" ></i> &nbsp;{{$child->id}}

                    </small>
                    <br>
                    <div style="background: #FFFFFF;padding: 5px 5px;margin-left: -5px;">{{$child->details}}</div>
...
<div class="direct-chat-btn bg-white" style="width: 100px; height: auto; display:inline-block">
    <a href="{{url('/')}}/uploads/<?php echo $path;?>" target="_blank"> <img src="{{url('/')}}/uploads/<?php echo $path;?>" style="width: 100%; height: auto; border: solid 1px #DDD;
padding: 5px;"></a>
</div>
<?php } ?>